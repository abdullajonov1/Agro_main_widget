...
const detachedQueryLayerAuthFailedUrls = new Set<string>();
let agriIdentityTokenRegistered = false;
...
function isAuthFailureMessage(err: unknown): boolean {
  const msg = String(
    (err as any)?.message ||
...
function isAgriMapLayerCandidate(layer: any): boolean {
  if (!isQueryableFieldLayer(layer)) return false;
 ...
 */
function buildMapDistrictClause(
  sublayer: any,
  tuman: string,
...
function isGroupSublayer(sublayer: any): boolean {
 ...
 */
function clearFieldLayerScaleLimits(layer: any): void {
  if (!layer) return;
  try {
 ...
 */
function clearScaleLimitsOnRegionYearTree(layer: any): void {
  if (!layer) return;
...
function regionYearLog(phase: string, detail?: Record<string, unknown>): void {
  try {
...
function pruneAgriQueryCache(now = Date.now()): void {
